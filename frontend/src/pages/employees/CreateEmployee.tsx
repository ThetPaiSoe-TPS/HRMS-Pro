...
export const CreateEmployee: React.FC = () => {
  ...
  useEffect(() => {
    // Generate employee code from backend
    employeeApi
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        general: error.response?.data?.message || "Failed to create employee",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-3xl p-4 mx-auto sm:p-6 lg:p-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Add Employee</h1>
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  {formData.position_id && (
                    <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                      {
                        positions.find(
                          (p) => p.id === parseInt(formData.position_id),
                        )?.title
                      }
                    </span>
                  )}
